...
This contract currently pins `soroban-sdk = 26.1.0` in `Cargo.toml`. Treat
Stellar's software version matrix as the source of truth for protocol and SDK
compatibility: build and test with the SDK line that matches the Stellar
network protocol you plan to deploy to.
...
| Protocol 26 | Mainnet, May 6 2026; Testnet, April 16 2026 | 26.0.1 in the Stellar matrix; this repo currently pins 26.1.0 | Current target for this repo. Keep the pinned SDK compatible with protocol 26 host behavior and run the full CI suite after any SDK bump. |
